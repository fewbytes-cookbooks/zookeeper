...
remote_file "/tmp/zookeeper-#{node[:zookeeper][:version]}.tar.gz" do
  source "http://mirrors.sonic.net/apache//zookeeper/zookeeper-#{node[:zookeeper][:version]}/zookeeper-#{node[:zookeeper][:version]}.tar.gz"
  mode "0644"
end
...
zk_servers += search(:node, "role:kafka OR role:zookeeper OR zookeeper_cluster_name:#{node[:zookeeper][:cluster_name]} NOT name:#{node.name}") # don't include this one, since it's already in the list
